...
import { Trophy, Tv, BarChart3, Sparkles, Zap, CheckCircle2, Shield, Star, Award, TrendingUp, Flame, ChevronLeft, ChevronRight, LogOut, User } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100 relative overflow-hidden flex flex-col justify-between selection:bg-amber-500 selection:text-black">
      ...
      {/* TOP NAVIGATION BAR: Sign In strictly positioned in the top-right */}
      <nav className="border-b border-white/10 bg-slate-950/80 backdrop-blur-xl sticky top-0 z-50 px-4 md:px-12 py-3.5 flex justify-between items-center shadow-2xl">
        ...
        {/* Right Nav Options with Single Sign In / Profile Button */}
        <motion.div 
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
      ...
      {/* SECTION 1: ALL 10 OFFICIAL IPL FRANCHISES (KEPT ABOVE PLAYERS) */}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3.5 md:gap-4">
          {IPL_TEAMS_PRESETS.map((team, idx) => (
            <motion.div
              ...
              className="glass-card glass-card-hover p-4 md:p-5 rounded-2xl border border-white/10 flex flex-col items-center text-center justify-between shadow-xl relative overflow-hidden group"
      ...
      {/* SECTION 2: 12 MARQUEE INDIAN SUPERSTARS (INFINITE LOOP SCROLLING ANIMATION) */}
            ...
            {loopingPlayers.map((player, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Player Name & Assigned Franchise */}
